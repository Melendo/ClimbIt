...
class DesuscribirseRocodromo {
  ...
  async execute({ escaladorApodo, idRocodromo }) {
    try {
      ...
      const estaSuscrito = await this.escaladorRepository.estaSuscrito(
        escaladorApodo,
        idRocodromo
      );
      if (!estaSuscrito) {
      ...
      return {
        mensaje: `Escalador ${escaladorApodo} desuscrito del rocódromo ${rocodromoEncontrado.nombre} exitosamente.`,
      };
    } catch (error) {
