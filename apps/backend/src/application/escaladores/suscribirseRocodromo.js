...
class SuscribirseRocodromo {
  ...
  async execute({ escaladorApodo, idRocodromo }) {
    try {
      ...
      const yaSuscrito = await this.escaladorRepository.estaSuscrito(
        escaladorApodo,
        idRocodromo
      );
      if (yaSuscrito) {
      ...
      await this.escaladorRepository.suscribirse(
        escaladorApodo,
        rocodromoEncontrado
      );
      return {
        mensaje: `Escalador ${escaladorApodo} suscrito al rocódromo ${rocodromoEncontrado.nombre} exitosamente.`,
      };
    } catch (error) {
