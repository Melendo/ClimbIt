  ...
  constructor(pistaRepository, escaladorRepository) {
    ...
    async execute({ idPista, nuevoEstado, escaladorApodo }) { 
        try {
            ...
            await this.pistaRepository.cambiarEstado(idPista, escalador.id, nuevoEstado);
            return { mensaje: `Estado de la pista con ID ${idPista} cambiado a ${nuevoEstado} exitosamente.` };
        } catch (error) {
            throw new Error(`Error al cambiar el estado de la pista: ${error.message}`);
        }
    }
}
