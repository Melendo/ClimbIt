...
describe('BuscarEscaladoresPorNombre Use Case', () => {
  ...
  it('debe devolver la lista de escaladores excluyendo información sensible', async () => {
    const mockEscaladores = [
      { id: 1, apodo: 'alex_honnold', correo: 'alex@test.com', contrasena: '123', idFotoPerfil: 5 },
      { id: 2, apodo: 'alex_megos', correo: 'megos@test.com', contrasena: '123', idFotoPerfil: null },
    ];

    mockEscaladorRepository.buscarPorApodoSimilitud.mockResolvedValue(mockEscaladores);

    const resultado = await useCase.execute('alex', 'exclude_apodo');

    expect(mockEscaladorRepository.buscarPorApodoSimilitud).toHaveBeenCalledWith('alex', 10, 'exclude_apodo');
    expect(resultado).toHaveLength(2);
    expect(resultado[0]).toEqual({ id: 1, apodo: 'alex_honnold', idFotoPerfil: 5 });
    expect(resultado[1]).toEqual({ id: 2, apodo: 'alex_megos', idFotoPerfil: null });
  ...
  it('debería excluir el apodo proporcionado', async () => {
    ...
    const fakeEscaladores = [
      { id: 1, correo: 'a@b.com', contrasena: '123', apodo: 'Alex', activo: true },
    ];
    mockEscaladorRepository.buscarPorApodoSimilitud.mockResolvedValue(fakeEscaladores);

    const result = await useCase.execute('al', excludeApodo);

    expect(mockEscaladorRepository.buscarPorApodoSimilitud).toHaveBeenCalledWith('al', 10, excludeApodo);
