...
async function warmUpRocodromoDataCache(misRocodromos, headers) {
  ...
  const rocoResults = await Promise.allSettled(
    rocodromoEndpoints.map((endpoint) =>
  ...
  if (failedRoco > 0) {
    console.warn(
      `Warm-up de rocodromos completado con ${failedRoco} fallo(s).`
    );
  }
  ...
  for (const rocodromo of misRocodromos) {
    ...
    const zonasResult =
      zonasResultIndex > -1 ? rocoResults[zonasResultIndex] : null;
  ...
  if (failedZonas > 0) {
    console.warn(
      `Warm-up de zonas/rutas completado con ${failedZonas} fallo(s).`
    );
  }
  ...
  if (rutaEndpoints.length > 0) {
    ...
    const failedRutas = rutaResults.filter(
      (r) => r.status === 'rejected'
    ).length;
    if (failedRutas > 0) {
      console.warn(
        `Warm-up de info de rutas completado con ${failedRutas} fallo(s).`
      );
    }
  }
}
...
export async function warmUpAppDataCache() {
  ...
  if (failedUser > 0) {
    console.warn(
      `Warm-up de datos de usuario completado con ${failedUser} fallo(s).`
    );
  }
  ...
  if (logoEndpoints.length > 0) {
    ...
    const failedLogos = logoResults.filter(
      (r) => r.status === 'rejected'
    ).length;
    if (failedLogos > 0) {
