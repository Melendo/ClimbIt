...
export function showProfilePhotoModal({ photos = [], currentPhotoId = null } = {}) {
    return new Promise((resolve) => {
        ...
        const initiallySelectedPhoto = photos.find((photo) => Number(photo.id) === normalizedCurrentId) || fallbackPhoto;
        const initialSelectedId = initiallySelectedPhoto ? Number(initiallySelectedPhoto.id) : null;
        const optionsHtml = photos
            .map((photo) => buildPhotoOptionHtml(photo, Number(photo.id) === initialSelectedId))
            .join('');
