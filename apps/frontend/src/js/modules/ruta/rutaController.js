...
async function obtenerResumenValoracionPista(idPista) {
  try {
    ...
    console.warn(
      `No se pudo obtener la valoracion total de la pista ${idPista}:`,
      err.message
    );
    return {
      averageRating: 0,
...
async function obtenerRutasActivasZona(idZona, colorScaleMap = {}) {
  ...
  try {
    ...
    console.warn(
      `No se pudieron cargar las rutas activas de la zona ${idZona}:`,
      err.message
    );
    return [];
  }
  ...
  try {
    ...
    for (const rocodromo of rocodromosList) {
      ...
      try {
        ...
        console.warn(
          `No se pudieron cargar zonas del rocodromo ${idRocodromo}:`,
          err.message
        );
      }
    }
    ...
    console.warn(
      'No se pudieron cargar los rocodromos para resolver la zona:',
      err.message
    );
  }
...
async function canManageRutaByZonaId(idZona) {
  ...
  for (const idRocodromo of managedIds) {
    try {
      ...
      const match =
        Array.isArray(zonas) &&
        zonas.some((zona) => Number(zona.id) === Number(idZona));
      if (match) {
        return true;
      }
    } catch (err) {
      console.warn(
        `No se pudieron cargar zonas para validar permisos del rocódromo ${idRocodromo}:`,
        err.message
      );
    }
  }
...
function findConflictingRoute(
  point,
  rutas = [],
  { minDistance = MIN_ROUTE_SEPARATION, excludeRutaId = null } = {}
) {
  ...
  for (const ruta of rutas) {
    ...
    if (
      Number.isInteger(excludeRutaId) &&
      Number.isInteger(rutaId) &&
      rutaId === excludeRutaId
    ) {
      continue;
    }
...
function validateFields(
  ...
  const errors = {};
  const { minRouteSeparation = MIN_ROUTE_SEPARATION, excludeRutaId = null } =
    options;
  ...
  if (dificultad && !allowedDificultades.includes(dificultad)) {
    errors.dificultad =
      'La dificultad seleccionada no pertenece a la escala del tipo elegido';
  }
  ...
  if (values.fechaRetirada && !fechaRetiradaIso) {
    ...
    errors.fechaRetirada =
      'La fecha de retirada debe ser posterior al momento actual';
  }

  if (!selectedPoint) {
    errors.posicion =
      'Selecciona una posicion en el mapa para guardar posX y posY';
  } else {
    const conflictingRoute = findConflictingRoute(
      selectedPoint,
      existingRutas,
      {
    ...
    if (conflictingRoute) {
      errors.posicion =
        'La posicion seleccionada se superpone con otra ruta. Selecciona un punto cercano, pero no encima.';
    }
  }
...
export async function crearRutaCmd(container, params = {}) {
  ...
  const hasValidParams =
    Number.isInteger(idRocodromo) &&
    idRocodromo > 0 &&
    Number.isInteger(idZona) &&
    idZona > 0;

  let nombreRocodromo = hasValidParams
    ? `Rocodromo ${idRocodromo}`
    : 'Rocodromo';
  ...
  if (!hasValidParams) {
    contextError =
      'La URL debe incluir idRocodromo e idZona validos para crear una ruta.';
  } else {
    try {
    ...
    try {
      ...
      if (!zonaActual) {
        contextError =
          'La zona indicada no pertenece a este rocodromo o no existe.';
      } else {
    ...
    try {
      ...
      dificultadOptionsByTipo = {
        boulder: buildDificultadOptions(
          escalas?.escalaDificultadBloque,
          colorScaleMap
        ),
        via: buildDificultadOptions(
      ...
      console.warn(
        'No se pudieron cargar las escalas de dificultad del rocodromo:',
        err.message
      );
    }
  }
  ...
  const applyDificultadOptionsByTipo = (
    tipo,
    { dificultadSelect, setDificultadOptions }
  ) => {
  ...
  const callbacks = {
    ...
    onViewReady: ({
      ...
      const selectedTipo = tipoBoulderInput.checked
        ...
        : tipoViaInput.checked
          ? 'via'
          : '';
      applyDificultadOptionsByTipo(selectedTipo, {
        dificultadSelect,
        setDificultadOptions,
      });
        ...
        onMapPointSelect: (point) => {
          ...
          if (conflictingRoute) {
            showFormAlert(
              alertBox,
              'warning',
              'Ese punto está encima de otra ruta. Puedes colocarla más cerca que antes, pero sin superponerla.'
            );
            return;
          }
      ...
      mapaSelector
        ...
        .catch((err) => {
          ...
          showFormAlert(
            alertBox,
            'danger',
            'No se pudo cargar el mapa para seleccionar la posicion.'
          );
        });
    },
    ...
    onTipoChange: (
      tipo,
      { dificultadSelect, setDificultadOptions, alertBox }
    ) => {
      applyDificultadOptionsByTipo(tipo, {
        dificultadSelect,
        setDificultadOptions,
      });
    ...
    onSubmit: async (values, fields) => {
      ...
      if (!zonaMapaSvg) {
        showFormAlert(
          alertBox,
          'warning',
          'No hay mapa para esta zona, no se puede seleccionar la ubicacion.'
        );
        if (submitButton) {
          submitButton.disabled = true;
        }
      ...
      // Validar campos
      const allowedDificultades = (
        dificultadOptionsByTipo[values.tipo] || []
      ).map((option) => option.value);
      const allowedColorPresas = colorPresasOptions.map(
        (option) => option.value
      );
      ...
      if (Object.keys(errors).length > 0) {
        ...
        if (errors.dificultad)
          setFieldError(dificultadSelect, errors.dificultad);
        if (errors.colorPresas)
          setFieldError(colorPresasSelect, errors.colorPresas);
        if (errors.tipo) {
          setTipoFieldError(container, errors.tipo);
        }
        if (errors.fechaCreacion)
          setFieldError(fechaCreacionInput, errors.fechaCreacion);
        if (errors.fechaRetirada)
          setFieldError(fechaRetiradaInput, errors.fechaRetirada);
...
export async function infoRutaCmd(container, id) {
  ...
  try {
    ...
    const zonaContext =
      Number.isInteger(idZonaRuta) && idZonaRuta > 0
        ? await resolveRocodromoContextByZonaId(idZonaRuta)
        : null;

    if (
      zonaContext?.idRocodromo &&
      Number.isInteger(idZonaRuta) &&
      idZonaRuta > 0
    ) {
    ...
    ruta.colorPresasRgb = resolveColorScaleRgb(
      ruta?.colorPresas,
      colorScaleMap
    );
    ...
    if (zonaContext?.idRocodromo) {
      try {
        const escalasRes = await fetchClient(
          `/rocodromos/${zonaContext.idRocodromo}/escalasDificultad`
        );
    ...
    const callbacks = {
      ...
      onEstadoChange: async (estado, estadoElement, estadoTextoElement) => {
        ...
        const estadoBackend =
          ESTADOS_BACKEND[ESTADOS_FRONTEND[estado] || estado] || 'S/N';
      ...
      onRatingSave: async (selectedStars) => {
        if (!canRateRutaByEstado(ruta.estado)) {
          throw new Error(
            'Solo puedes valorar rutas marcadas como completadas o flash.'
          );
        }
        ...
        try {
          ...
          showToast('Valoracion guardada correctamente.', {
            variant: 'success',
          });
...
export async function modificarRutaCmd(container, id) {
  ...
  try {
    ...
    if (!hasValidParams) {
      contextError =
        'No se pudo resolver el rocódromo/zona de la ruta a modificar.';
    ...
    let selectedPoint = null;
    if (
      Number.isFinite(Number(ruta.posX)) &&
      Number.isFinite(Number(ruta.posY))
    ) {
      selectedPoint = {
        x: Number(ruta.posX),
        y: Number(ruta.posY),
    ...
    const callbacks = {
      ...
      onViewReady: ({
        ...
        mapaSelector
          ...
          .catch((err) => {
            console.error(
              'No se pudo inicializar el mapa para modificar ruta:',
              err
            );
            showFormAlert(
              alertBox,
      ...
      onSubmit: async (values, fields) => {
        ...
        try {
          ...
          showFormAlert(
            alertBox,
            'danger',
            `Error al modificar ruta: ${err.message}`
          );
        } finally {
          clearSubmitLoading?.();
        }
