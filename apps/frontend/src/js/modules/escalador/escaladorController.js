  ...
  try {
    ...
    const callbacks = {
      ...
      onUpdateDescripcion: async (descripcion) => {
        try {
          const response = await fetchClient(
            '/escaladores/actualizarDescripcion',
            {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
          ...
          showToast('Descripcion actualizada correctamente.', {
            variant: 'success',
          });
        } catch (err) {
          const errorMsg = await extractValidatorMessage(err);
          showToast(
            `No se ha podido completar el cambio: ${errorMsg || err.message}`
          );
        }
      },
      onUpdateApodo: async (apodo) => {
        try {
          ...
          if (!validation.ok) {
            showToast(
              `No se ha podido completar el cambio: ${validation.message}`
            );
            return;
          }
