...
import {
  renderRutasProgressBar,
  calcularRutasCompletadas,
} from '../../components/rutasProgressBar.js';
...
export function renderMapaZona(
  ...
  const setupImageLazyLoading = (requestId) => {
    ...
    const lazyImages = Array.from(
      rutasContainer.querySelectorAll(
        'img[data-ruta-id][data-has-image="true"]'
      )
    );
  ...
  const updateFilterBadge = () => {
    const hasTipo =
      filterState.tipo === 'boulder' || filterState.tipo === 'via';
    const hasRango = Boolean(
      filterState.dificultadMin || filterState.dificultadMax
    );
  ...
  const syncDificultadRange = () => {
    ...
    const minIndex = filterState.dificultadMin
      ? indexMap.get(filterState.dificultadMin)
      : null;
    const maxIndex = filterState.dificultadMax
      ? indexMap.get(filterState.dificultadMax)
      : null;
  ...
  const updateFilterForm = () => {
    ...
    if (!hasTipo) {
      ...
      filtroHint.textContent =
        'Selecciona tipo para habilitar el rango de dificultad.';
    } else if (options.length === 0) {
      filterState.dificultadMin = '';
      filterState.dificultadMax = '';
      filtroHint.textContent =
        'No hay escala de dificultad configurada para este tipo.';
    } else {
      syncDificultadRange();
      filtroHint.textContent =
        'Se usa el orden de la escala del rocódromo (de más fácil a más difícil).';
    }

    if (
      filterState.dificultadMin &&
      options.some((opt) => opt.value === filterState.dificultadMin)
    ) {
    ...
    if (
      filterState.dificultadMax &&
      options.some((opt) => opt.value === filterState.dificultadMax)
    ) {
  ...
  const loadZonas = async (idZona, direction = null) => {
    ...
    const progressBarHtml = renderRutasProgressBar(
      rutas.length,
      rutasCompletadas
    );
  ...
  const modalInstance =
    zonaFiltrosModalEl && window.bootstrap
